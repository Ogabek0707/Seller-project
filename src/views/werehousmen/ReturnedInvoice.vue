      ...
      <div class="invoice">
        <div class="add">
            <div class="input_bar-info">
                <span>{{ returned.title }}</span>
                <span>{{ status.total_sum }}</span>
            </div>
            <button type="submit" @click="Add" v-if="status.status == 'new'">Add Returned Item</button>
        </div>
        ...
        <div class="pagination">
            ...
            <div class="actions" v-if="status.status == 'new'">
...
const getProduct = () => {
    http.get('warehouse/returned-invoice-item/', {
        params: {
...
const DeleteP = () => {
    http.delete(`warehouse/returned-invoice-item/${code.value}/`).then(res => {
        Notification({text: "Deleted Returned Item !!!"}, {type: "danger"})
        setTimeout(()=>{
            location.reload()
        },2000)
    }).catch(err =>{
...
const EditItem = (e) => {
    e.preventDefault()
    http.put(`warehouse/returned-invoice-item/${code.value}/`,{
        quantity: product_list2.value.quantity,
        price: product_list2.value.price,
    }).then(res => {
        console.log(res);
        Notification({text: 'Edit Returned Item !!!'}, {type: 'warning'})
        setTimeout(() => {
            location.reload()
        },2000)
    }).catch(err => {
...
const Confirm = () => {
    http.post(`warehouse/returned-invoice/${route.params.id}/confirm/`).then(res => {
        console.log(res);
        Notification({text: 'Returned Confirmed !!!'},{type: 'warning'})
        router.push({name: 'returned'})
    }).catch(err => {
        console.log(err);
...
const Delete = () => {
    http.delete(`warehouse/returned-invoice/${route.params.id}/`).then(res => {
      console.log(res);
      Notification({text: "Returned Deleted  !!!"}, {type: "danger"})
      location.reload()
    }).catch(err =>{
      console.log(err);
       Notification({text: "Something wrong  !!!"}, {type: "danger"})
...
const getInvoice = () => {
    http.get(`warehouse/returned-invoice/${route.params.id}`).then(res => {
        console.log(res.data);
        status.value = res.data
        returned.value = res.data.client
    })
}
onMounted(()=>{
      getProduct()
