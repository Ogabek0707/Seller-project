<template>
      <div class="invoice">
        <div class="add">
            ...
            <button type="submit" @click="Add" v-if="status.status == 'new'">Add Output invoice</button>
        </div>
...
const getProduct = () => {
    http.get('warehouse/output-invoice-item/', {
        params: {
...
const DeleteP = () => {
    http.delete(`warehouse/output-invoice-item/${code.value}/`).then(res => {
        Notification({text: "Deleted Output Item !!!"}, {type: "danger"})
        setTimeout(()=>{
            location.reload()
        },2000)
    }).catch(err =>{
...
const AddItem = (e) => {
    e.preventDefault();
    http.post('warehouse/output-invoice-item/',{
...
const EditItem = (e) => {
    e.preventDefault()
    http.put(`warehouse/output-invoice-item/${code.value}/`,{
...
const Confirm = () => {
    http.post(`warehouse/output-invoice/${route.params.id}/confirm/`).then(res => {
        console.log(res);
        Notification({text: 'Output Confirmed !!!'},{type: 'warning'})
        router.push({name: 'delivered'})
    }).catch(err => {
        console.log(err);
...
const Delete = () => {
    http.delete(`warehouse/output-invoice/${route.params.id}/`).then(res => {
      console.log(res);
      Notification({text: "Output Deleted  !!!"}, {type: "danger"})
      location.reload()
    }).catch(err =>{
      console.log(err);
       Notification({text: "Something wrong  !!!"}, {type: "danger"})
...
const getInvoice = () => {
    http.get(`warehouse/output-invoice/${route.params.id}`).then(res => {
